...
from basic_function.calc_Greek import calc_put_delta, calc_gamma, calc_vega, calc_theta_put
from basic_function.calc_implied_volatility import put_implied_vol


# 连接数据库，选出所有交易日
def get_trade_date(start_date, end_date, option_string):
    data_path = r'D:\pc\Desktop\Algo_Trading\Assignment2\data\raw\option.db'
...
def option_delta_allocation(delta_total, close_total, k_total, wind_id_total, iv_total, a, T, R, s0):
    if min(delta_total) < a < max(delta_total):
        ...
        theta_left = calc_theta_put(s0, k_left, T, iv_left, R)
        theta_right = calc_theta_put(s0, k_right, T, iv_right, R)
        i = 1
        j = 1
...
def get_option_close(trade_date, option_end_date, wind_id):
    data_path = r'D:\pc\Desktop\Algo_Trading\Assignment2\data\raw\option.db'
    my_db = sqlite3.connect(data_path)
    c = my_db.cursor()
    sql = f"SELECT K,CLOSE \nfrom OPTION_LOCAL ol\nwhere TRADE_DT = {trade_date} and CALL_PUT='put' " \
          f"and OPTION_ENDTRADE = {option_end_date} and wind_id='{wind_id}'\norder by K ASC"
    c.execute(sql)
    result = c.fetchall()
    close = result[0][1]
    return close
...
def option_after_date_change(option, trade_date, option_end_date, s0, T, R):
    if option.loc['left', 'k'] != 0 and option.loc['right', 'k'] != 0:
        option.loc['left', 'close'] = get_option_close(trade_date, option_end_date, option.loc['left', 'wind_id'])
        option.loc['left', 'iv'] = put_implied_vol(s0, option.loc['left', 'k'], T, option.loc['left', 'close'], R)
        option.loc['left', 'delta'] = calc_put_delta(s0, option.loc['left', 'k'], T, option.loc['left', 'iv'], R)
        ...
        option.loc['left', 'theta'] = calc_theta_put(s0, option.loc['left', 'k'], T, option.loc['left', 'iv'], R)
        option.loc['right', 'close'] = get_option_close(trade_date, option_end_date, option.loc['right', 'wind_id'])
        option.loc['right', 'iv'] = put_implied_vol(s0, option.loc['right', 'k'], T, option.loc['right', 'close'], R)
        option.loc['right', 'delta'] = calc_put_delta(s0, option.loc['right', 'k'], T, option.loc['right', 'iv'], R)
        ...
        option.loc['right', 'theta'] = calc_theta_put(s0, option.loc['right', 'k'], T, option.loc['right', 'iv'], R)
...
def skew_arbitrage_put(start_date = 20190701,end_date = 20210330):
    R = 0.02
    option_string = '510050.SH'
    data_path = r"D:\pc\Desktop\Algo_Trading\Assignment2\result\data\put_iv_diff_up_mid_low.csv"
    up_middle_low = pd.read_csv(data_path, index_col=0)
    up_middle_low = up_middle_low.to_dict()

    # 连接数据库，选出所有交易日
    trade_dt_total = get_trade_date(start_date, end_date, option_string)
    ...
    # 读取vix指数
    data_path_2 = r"D:\pc\Desktop\Algo_Trading\Assignment2\data\raw\vix.csv"
    ...
    for trade_date in trade_dt_total:
        ...
        # 如果下一个交易日距离到期日仅剩不到五天的时间，那么选择不开仓
        # trade_date_next_day = trade_dt_total[trade_dt_total.index(trade_date) + 1]
        # days_2 = (parse(str(option_end_date)) - parse(str(trade_date_next_day))).days
        # if days_2 <= 4:
        #     state = 0
        #     state_all.append(state)
        #     print('看这里')
        #     print(f'交易日：{trade_date}')
        #     print(f'下一个交易日：{trade_date_next_day}')
        #     print(f'到期日：{option_end_date}')
        #     continue
        ...
        if condition_1 or condition_4 or condition_5:  # 买入-0.5，卖出-0.25
            print('开仓1')
    ...
    axs[1].plot(trade_date_all, [etf_daily_close_df.loc[i, 'close'] for i in trade_dt_total], linewidth=2, color='black')
    axs[1].set_ylabel('s0')
    ...
    open_close_df.to_csv(r'D:\pc\Desktop\Algo_Trading\Assignment2\result\data\open_close_detail(put).csv')
    open_close_total_df.to_csv(r'D:\pc\Desktop\Algo_Trading\Assignment2\result\data\open_close_total(put).csv')

    return trade_date_all, balance_all

if __name__=='__main__':
    skew_arbitrage_put(20190701,20210330)
